...
describe('exporters', () => {
  ...
  describe('pdfExporter', () => {
    it('exports pdf using toPng approach', async () => {
      ...
      const result = await pdfExporter.export(el, makeOptions({ format: 'pdf', filename: 'vision.pdf' }));
      expect(result.success).toBe(true);
    ...
    it('handles pdf export error when toPng fails', async () => {
      ...
      const result = await pdfExporter.export(el, makeOptions({ format: 'pdf', filename: 'fail.pdf' }));
