...
describe("useKeyboardShortcuts", () => {
  ...
  it("calls onShowShortcuts on Ctrl+Shift+/", () => {
    ...
    handler(
      new KeyboardEvent("keydown", { key: "/", shiftKey: true, ctrlKey: true }),
    );
    expect(onShowShortcuts).toHaveBeenCalled();
  });
  ...
  it("calls onRedo on Ctrl+Shift+Z", () => {
    ...
    handler(
      new KeyboardEvent("keydown", { key: "z", shiftKey: true, ctrlKey: true }),
    );
...
describe("buildDefaultShortcutGroups", () => {
  ...
  it("omits undo/redo when feature disabled", () => {
    const noUndo = {
      ...config,
      features: { ...config.features, undoRedo: false },
    };
  ...
  it("omits export when feature disabled", () => {
    const noExport = {
      ...config,
      features: { ...config.features, export: false },
    };
  ...
  it("omits sidebar toggle when feature disabled", () => {
    const noSidebar = {
      ...config,
      features: { ...config.features, sidebar: false },
    };
    const groups = buildDefaultShortcutGroups(noSidebar);
    const view = groups.find((g) => g.title === "View");
    expect(view?.shortcuts.some((s) => s.label === "Toggle sidebar")).toBe(
      false,
    );
  });
  ...
  it("omits dark mode toggle when feature disabled", () => {
    const noDark = {
      ...config,
      features: { ...config.features, darkMode: false },
    };
    const groups = buildDefaultShortcutGroups(noDark);
    const view = groups.find((g) => g.title === "View");
    expect(view?.shortcuts.some((s) => s.label === "Toggle dark mode")).toBe(
      false,
    );
  });
