...
describe("useDragAndDropImport", () => {
  it("sets dragging state on enter/leave", () => {
    ...
    act(() => {
      window.dispatchEvent(
        withDataTransfer(new Event("dragenter", { bubbles: true })),
      );
    });
    ...
    act(() => {
      window.dispatchEvent(
        withDataTransfer(new Event("dragleave", { bubbles: true })),
      );
    });
    expect(result.current.isDragging).toBe(false);
  });
  ...
  it("imports accepted file by extension", () => {
    ...
    act(() => {
      window.dispatchEvent(
        withDataTransfer(new Event("dragenter", { bubbles: true }), file),
      );
      window.dispatchEvent(
        withDataTransfer(new Event("drop", { bubbles: true }), file),
      );
    });
