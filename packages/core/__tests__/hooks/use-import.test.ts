...
describe("useImport", () => {
  it("imports a valid .itsjust.json file", async () => {
    ...
    let importResult:
      | Awaited<ReturnType<typeof result.current.importFromFile>>
      | undefined;
    await act(async () => {
  ...
  it("rejects unsupported formats", async () => {
    const { result } = renderHook(() =>
      useImport({ acceptedFormats: ["json"] }),
    );
  ...
  it("rejects files exceeding maxFileSize", async () => {
    const { result } = renderHook(() =>
      useImport({ acceptedFormats: ["json"], maxFileSize: 10 }),
    );
  ...
  it("handles .itsjust.json parse error", async () => {
    ...
    const file = new File(["not json"], "test.itsjust.json", {
      type: "application/json",
    });
  ...
  it("returns error when importFromEvent has no file", async () => {
    ...
    let importResult:
      | Awaited<ReturnType<typeof result.current.importFromEvent>>
      | undefined;
    await act(async () => {
