...
export function buildDefaultShortcutGroups(
  config: ToolConfig,
): ShortcutGroup[] {
  ...
  if (config.features.export)
    actions.push({ keys: `${mod}+E`, label: "Export" });
  ...
  if (config.features.sidebar)
    view.push({ keys: `${mod}+B`, label: "Toggle sidebar" });
  if (config.features.darkMode)
    view.push({ keys: `${mod}+D`, label: "Toggle dark mode" });
